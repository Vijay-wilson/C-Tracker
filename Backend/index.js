...
// admin delete
app.delete("/curricul/delete/:id", async(req, res) => {
  try {
...
// approve curriculum
app.put("/curupdate/:id", async(req, res) => {
  try {
    let item = req.body
...
// Download Requirement uploads
    app.get("/reqfiledownload/:id", async(req,res)=>{
      let id=req.params.id
      console.log(id);
...
app.get("/curfileopen/:id", async (req, res) => {
...
// ======
// viewing req uploads
app.get("/reqfileopen/:id", async (req, res) => {
  const id = req.params.id;
  const list = await ReqModel.findById({ _id: id });
  const pdfpath = path.join(__dirname + "/uploads/" + list.pdfpath);
  res.sendFile(pdfpath);
